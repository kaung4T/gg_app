...
function filterTree(data: RouteComponent[]) {
  const newTree = cloneDeep(data).filter(
    (v: { meta: { showLink: boolean; Developcomplete?: boolean } }) =>
      v.meta?.showLink !== false && v.meta.Developcomplete != true
  );
  newTree.forEach(
...
const translateLan = list => {
  ...
  list.forEach(item => {
    ...
    item.meta.title = lan === 'ko' ? item.meta.title : item.meta.enTitle;
  });
};
...
const transformItem = (item: UserAPI.RouterResData) => {
  ...
  const hasChildrenWithZeroType = item.childResourceList.some(
    child => child.type === 0
  );
  if (hasChildrenWithZeroType) {
    (baseItem as any).children = item.childResourceList.map(transformItem);
  } else {
    baseItem.meta.childResourceList = item.childResourceList.map(
      grandChildItem => ({
        ...grandChildItem,
...
function initRouter() {
  if (getConfig()?.CachingAsyncRoutes) {
    ...
    return new Promise(resolve => {
      const userInfo =
        storageSession().getItem<
          DataInfo<{ id: number; name: string; resources: null }>
        >(sessionKey);
      const permissionRoute = [];
      userInfo.roles.forEach(item => {
        if (permissionData[item.name]) {
          permissionRoute.push(...permissionData[item.name]);
        }
      });
      handleAsyncRoutes(cloneDeep(permissionRoute));
      resolve(router);
    });
  }
}
    ...
    if (v.meta?.frameSrc) {
      ...
      const index = v?.component
        ...
        : modulesRoutesKeys.findIndex(ev =>
            ev.includes(`/src/views${path}/index.vue`)
          ); // -index.vue文件作为路由文件
...
function getAuths(): Array<string> {
  return (router.currentRoute.value.meta.childResourceList as any[]).map(
    item => item.enName
  );
